...
**Deployment, enablement and restart require separate approval.** `Database__TokenRenewalDiagnostics__Enabled=true` is opt-in; absent/false installs no instrumentation/query. Only trusted host `Development` enables it (case-insensitive); elsewhere it reports `Disabled` without changing database behavior. Invalid boolean values fail configuration. Development connection-string fallback/local containers report `Unavailable`/not tested. Credential precedence, bootstrap/design-time paths, pooling, refresh intervals (55 minutes / 5 seconds) and endpoints remain unchanged.

Information-level JSON under `Api.Configurations.PostgresTokenDiagnostics` carries opaque process/data-source/physical IDs and local/PostgreSQL PIDs. `ProviderStarted`/`ProviderCompleted` carry sequence, start/completion UTC and **provider** `ExpiresOn`, not startup-preflight expiry. `ExpiryAdvanced` compares expiry only, not token identity/cache/renewal. `PhysicalAuthenticated` observes [Npgsql 10.0.2's new-physical initializer](https://github.com/npgsql/npgsql/blob/v10.0.2/src/Npgsql/Internal/NpgsqlConnector.cs#L488-L573) after authentication/bootstrap, never logical pool checkout. `PhysicalReadSucceeded` follows the same connector's fixed read of constant `1` and its own `pg_stat_activity.backend_start`, filtered by `pg_backend_pid()`: `BackendStartedUtc` is server-clock time; `ObservedUtc` is application-clock time. No token text/hash/claims, connection strings, user/database names, parameters or exception contents are logged by these diagnostics; diagnostic state retains no token material.

Native sync/async commands have a five-second timeout; async also has an independent five-second deadline. The initializer cannot receive the original open-cancellation token; driver cancellation/cleanup can add latency. Readers/commands are disposed. Exactly one row, constant `1` and a non-null finite UTC timestamp are required. Failures/cancellation propagate with failure observations, never read-success. [Own-session statistics](https://www.postgresql.org/docs/17/monitoring-stats.html#MONITORING-PG-STAT-ACTIVITY-VIEW) require no additional PostgreSQL privileges; unavailable metadata fails explicitly.

After authorized rollout, establish a **new baseline** on the workload-identity deployment (events do not identify the credential-chain member). Retain first provider expiry **E1**, stable process/data-source IDs and externally recorded pod UID/PID/zero restarts. Require later provider expiry **E2 > E1** and beyond observation time, plus a new physical ID's `PhysicalAuthenticated`/`PhysicalReadSucceeded` with **server `BackendStartedUtc` strictly after E1**, allowing for server/app/Entra clock uncertainty. A delayed initializer timestamp alone is insufficient. This is bounded in-process evidence, **not exact token-to-login attribution**: `TokenAttribution` is always `Unknown`; provider snapshots/pending counts cannot eliminate assignment/authentication races.

Use natural churn and a bounded window based on actual expiry (potentially 1-3 hours, never assumed TTL). Missing logs, fallback, restart, uncertain clock boundaries or no qualifying new session/later expiry mean **inconclusive**. Never clear pools, kill sessions or force reconnects. Callback counts, logical EF opens, generic traces, old-session reads and standalone token subprocesses are not substitutes; uninstrumented processes cannot prove this retrospectively.
