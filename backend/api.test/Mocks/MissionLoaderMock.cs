...
namespace Api.Test.Mocks
{
    public class MockMissionLoader() : IMissionLoader
    {
        ...
        public async Task<MissionDefinition?> GetMissionById(string missionId)
        {
            await Task.Run(() => Thread.Sleep(1));
            return _mockMissionDefinition;
        }

        public async Task<List<MissionDefinition>> GetAvailableMissions(string? installationCode)
        {
            await Task.Run(() => Thread.Sleep(1));
            return new List<MissionDefinition>([_mockMissionDefinition]);
        }
