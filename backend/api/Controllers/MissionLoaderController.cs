...
namespace Api.Controllers
{
    ...
    public class MissionLoaderController(ILogger<MissionLoaderController> logger, IMissionLoader missionLoader, IRobotService robotService) : ControllerBase
    {
        ...
        public async Task<ActionResult<IList<MissionDefinition>>> GetAvailableMissions([FromRoute] string? installationCode)
        {
            try
            {
                var missions = await missionLoader.GetAvailableMissions(installationCode);
                return Ok(missions);
            }
        ...
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [ProducesResponseType(StatusCodes.Status502BadGateway)]
        public async Task<ActionResult<MissionDefinition>> GetMissionDefinition([FromRoute] string missionId)
        {
            try
            {
