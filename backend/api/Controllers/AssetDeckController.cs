...
public class AssetDeckController : ControllerBase
{
    ...
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public async Task<ActionResult<AssetDeck>> Create(
        [FromBody] CreateAssetDeckQuery assetDeck
    )
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Succesfully created new asset deck with id '{assetDeckId}'", newAssetDeck.Id);
            return CreatedAtAction(nameof(GetAssetDeckById), new { id = newAssetDeck.Id }, newAssetDeck);
        }
        catch (Exception e)
        {
