...
namespace Api.Services
{
    ...
    {
        public Task<PagedList<Area>> ReadAll(
        ...
        public Task<IEnumerable<Area?>> ReadByInspectionAreaId(
            string inspectionAreaId,
            bool readOnly = true
        );

        public Task<Area?> ReadByInstallationAndName(
    ...
    {
        public async Task<PagedList<Area>> ReadAll(
            AreaQueryStringParameters parameters,
            bool readOnly = true
        )
        {
        ...
        public async Task<Area?> ReadByInstallationAndName(
            string installationCode,
            string areaName,
            bool readOnly = true
        )
        ...
        public async Task<Area> Create(CreateAreaQuery newAreaQuery)
        {
            var installation =
                await installationService.ReadByInstallationCode(
                    newAreaQuery.InstallationCode,
                    readOnly: true
                )
                ?? throw new InstallationNotFoundException(
                    $"No installation with name {newAreaQuery.InstallationCode} could be found"
                );

            var plant =
                await plantService.ReadByInstallationAndPlantCode(
                    installation,
                    newAreaQuery.PlantCode,
                    readOnly: true
                )
                ?? throw new PlantNotFoundException(
                    $"No plant with name {newAreaQuery.PlantCode} could be found"
                );
            ...
            var inspectionArea =
                await inspectionAreaService.ReadByInstallationAndPlantAndName(
                    installation,
                    plant,
                    newAreaQuery.InspectionAreaName,
                    readOnly: true
                )
                ?? throw new InspectionAreaNotFoundException(
                    $"No inspection area with name {newAreaQuery.InspectionAreaName} could be found"
                );
            ...
            if (existingArea != null)
            {
                throw new AreaExistsException(
                    $"Area with name {newAreaQuery.AreaName} already exists"
                );
            }
        ...
        public async Task<Area?> ReadByInstallationAndPlantAndInspectionAreaAndName(
            Installation installation,
            Plant plant,
            InspectionArea inspectionArea,
            string areaName,
        ...
        {
            Expression<Func<Area, bool>> installationFilter = string.IsNullOrEmpty(
                parameters.InstallationCode
            )
                ? area => true
                : area =>
                    ...
                    && area.InspectionArea.Plant.Installation.InstallationCode.ToLower()
                        .Equals(parameters.InstallationCode.ToLower().Trim());

            Expression<Func<Area, bool>> inspectionAreaFilter = area =>
                string.IsNullOrEmpty(parameters.InspectionArea)
                || (
                    area.InspectionArea != null
                    && area.InspectionArea.Name != null
                    && area.InspectionArea.Name.ToLower()
                        .Equals(parameters.InspectionArea.ToLower().Trim())
                );
