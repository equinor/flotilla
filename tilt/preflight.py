...
# The broker owns the local credentials, and setup.sh calls the same script, so
# the interactive and the Tilt route share one set whichever produced it. It
# writes broker/.local-credentials and broker/.env, both gitignored.
CREDENTIALS_SCRIPT = os.path.join(
    BROKER_DIR, "scripts", "ensure-local-credentials.sh"
)
...
def check_mqtt_credentials() -> Check:
    ...
    deployment also uses, and lets the stack run with TLS on. The script
    regenerates only when they are missing or close to expiry: the passwords
    reach the clients through the Tilt environment, so rotating them restarts
    every one of them.
    """
    if not os.path.isfile(CREDENTIALS_SCRIPT):
        return Check(
            "mqtt-credentials", False, "missing %s" % CREDENTIALS_SCRIPT, "Repository"
        )

    result = _run([CREDENTIALS_SCRIPT, "-q"], timeout=120)
    if result is None or result.returncode != 0:
        detail = result.stderr.strip() if result is not None else "openssl or bash missing"
        return Check(
            "mqtt-credentials",
            False,
            "ensure-local-credentials.sh failed: %s" % (detail or "no error details"),
            "Repository",
        )

    if result.stdout.strip() == "generated":
        return Check(
